...
final readonly class ReleaseStockReservationAction
{
    ...
    public function execute(StockReservation $reservation, array $attributes, User $actor): StockReservation
    {
        ...
        $payload = ['type' => InventoryMovementType::Release->value, 'reservation_id' => (int) $reservation->getKey(), 'lines' => $inputLines];
        $operationId = (string) $attributes['idempotency_key'];
        $expectedHash = (new InventoryMovementCommand(type: InventoryMovementType::Release, lines: [], operationId: $operationId, idempotencyPayload: $payload))->requestHash();
        ...
        return DB::transaction(function () use ($reservation, $actor, $inputLines, $payload, $operationId, $expectedHash): StockReservation {
            ...
            if (in_array($locked->status, [StockReservationStatus::Released, StockReservationStatus::Consumed], true)) {
                throw new InventoryConflict('La reserva ya no admite liberaciones.');
            }
            ...
            foreach ($reservationLines as $line) {
                ...
                if ($amount->isNegative() || $amount->isZero() || $amount->isGreaterThan($remaining)) {
                    throw new InventoryConflict('La cantidad a liberar supera la cantidad aún reservada.');
                }
                $line->forceFill(['released_quantity' => (string) BigDecimal::of((string) $line->released_quantity)->plus($amount)])->save();
                $movementLines[] = ['product_id' => (int) $line->product_id, 'stock_location_id' => (int) $line->stock_location_id, 'on_hand_delta' => '0', 'reserved_delta' => '-'.(string) $amount];
            }

            $movement = $this->recordMovement->execute(new InventoryMovementCommand(type: InventoryMovementType::Release, lines: $movementLines, operationId: $operationId, stockReservationId: (int) $locked->getKey(), idempotencyPayload: $payload), $actor);
            $locked->forceFill(['status' => $this->statusAfterChange($locked)])->save();
            $this->auditRecorder->record(AuditEntryData::forSubject(subject: $locked, actor: $actor, logName: 'inventory', event: 'stock_reservation_released', description: 'Stock liberado', operationId: $operationId, traceId: null, source: 'api', upId: null, properties: ['movement_id' => $movement->getKey(), 'result' => 'success']));
    ...
    private function statusAfterChange(StockReservation $reservation): StockReservationStatus
    {
        ...
        return $remaining ? StockReservationStatus::Released : StockReservationStatus::PartiallyConsumed;
    }
}
