...
final readonly class CancelEggStockTransactionAction
{
    ...
    public function execute(EggStockTransaction $transaction, array $data, User $actor, string $source = 'api'): array
    {
        ...
        return $this->commands->execute($actor, 'egg-stock.adjust', 'egg-stock.cancel', $data['idempotency_key'], ['transaction' => $transaction->public_id, ...$data], function (string $operationId) use ($transaction, $data, $actor, $source): array {
