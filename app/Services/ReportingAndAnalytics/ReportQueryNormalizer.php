...
namespace App\Services\ReportingAndAnalytics;
...
final readonly class ReportQueryNormalizer
{
    ...
    private function normalizeFilters(mixed $input, ReportSourceDefinition $definition): array
    {
        ...
        foreach ($input as $index => $filter) {
            if (! is_array($filter) || ! array_key_exists('field', $filter) || ! array_key_exists('operator', $filter)
                ...
                $this->invalid("filters.{$index}", 'Cada filtro debe indicar campo, operador y valor.');
            }

            $unknown = array_diff(array_keys($filter), ['field', 'operator', 'value']);
            if ($unknown !== []) {
                $this->invalid("filters.{$index}", 'Cada filtro solo puede indicar campo, operador y valor.');
            }
    ...
    /** @param array{label: string, type: string, operators: list<string>, options?: list<string>, options_source?: string} $definition */
    ...
    private function normalizeSorts(mixed $input, ReportSourceDefinition $definition): array
    {
        ...
        foreach ($input as $index => $sort) {
            if (! is_array($sort) || ! is_string($sort['field'] ?? null)
                ...
                $this->invalid("sorts.{$index}", 'Cada ordenamiento debe indicar campo y direccion válidos.');
            }

            $unknown = array_diff(array_keys($sort), ['field', 'direction']);
            if ($unknown !== []) {
                $this->invalid("sorts.{$index}", 'Cada ordenamiento solo puede indicar campo y direccion.');
            }
    ...
    {
        foreach (['available_stock', 'movement_count'] as $preferred) {
