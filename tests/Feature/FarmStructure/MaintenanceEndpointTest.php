...
final class MaintenanceEndpointTest extends TestCase
{
    ...
    public function test_creation_preserves_exact_cost_and_audits_without_changing_house(): void
    {
        ...
        $response = $this->withHeader('Idempotency-Key', Str::uuid()->toString())
            ...
            ->assertJsonPath('data.poultry_house_id', $house->id)
            ->assertJsonPath('data.maintenance_date', '2026-01-15')
            ->assertJsonPath('data.description', 'Reparación de bebederos')
    ...
    {
        // Preparación: mezcla fechas, statuses y un mantenimiento de otro galpón.
        $house = PoultryHouse::factory()->create();
        ...
        $response = $this->getJson($this->collectionUrl($house).'?per_page=2&date_from=2026-01-01')->assertOk()
        ...
        $this->assertStringContainsString('per_page=2', $nextPage);
        $this->assertStringContainsString('date_from=2026-01-01', $nextPage);
    ...
    public function test_correction_preserves_original_audit_and_rejects_stale_version(): void
    {
        ...
        $this->patchJson("/api/v1/maintenances/{$id}", [
    ...
    public function test_cancellation_preserves_history_and_has_no_delete_endpoint(): void
    {
        ...
        Sanctum::actingAs($actor, ['*']);
        $url = "/api/v1/maintenances/{$maintenance->id}";

        // Acción: valida el motivo y realiza la cancelación.
        $this->postJson($url.'/cancellation', ['version' => 1])->assertUnprocessable()->assertJsonValidationErrors('reason');
        $this->postJson($url.'/cancellation', ['version' => 1, 'reason' => 'Registro duplicado'])
            ->assertOk()->assertJsonPath('data.status', 'cancelled')->assertJsonPath('data.version', 2);
        $this->postJson($url.'/cancellation', ['version' => 2, 'reason' => 'Reintento'])->assertConflict();
    ...
    public function test_history_survives_responsible_rename_and_deactivation_without_unit_assignments(): void
    {
        ...
        $responsible->delete();
        $this->getJson("/api/v1/maintenances/{$id}")->assertOk()->assertJsonPath('data.responsible.name', 'Responsable original');
        $this->patchJson("/api/v1/maintenances/{$id}", ['version' => 1, 'reason' => 'Descripción', 'description' => 'Detalle corregido'])->assertOk();
        $this->withHeader('Idempotency-Key', Str::uuid()->toString())
    ...
    public function test_all_endpoints_require_authentication(): void
    {
        ...
        $this->postJson($url.'/cancellation', [])->assertUnauthorized();
    }
    ...
    public function test_http_endpoints_enforce_functional_permissions(): void
    {
        ...
        $this->postJson($url.'/cancellation', [])->assertForbidden();
    }
    ...
    public function test_audit_failure_rolls_back_business_change(string $operation): void
    {
        ...
        $response = match ($operation) {
            ...
            'update' => $this->patchJson("/api/v1/maintenances/{$maintenance->id}", [
                'version' => 1, 'reason' => 'Corrección', 'cost_amount' => '0.00',
            ]),
            'cancel' => $this->postJson("/api/v1/maintenances/{$maintenance->id}/cancellation", [
    ...
    public function test_missing_resources_return_404_and_house_cannot_be_reassigned(): void
    {
        ...
        $this->getJson('/api/v1/poultry-houses/999999/maintenances')->assertNotFound();
        $this->getJson('/api/v1/maintenances/999999')->assertNotFound();
        $this->patchJson("/api/v1/maintenances/{$maintenance->id}", [
    ...
    // Flujo: PostgreSQL impide costos negativos, statuses imposibles y pérdida de referencias históricas.
    ...
    {
        return ['cost' => ['cost'], 'status' => ['status'], 'cancelación incompleta' => ['cancel'], 'galpón' => ['house'], 'responsible' => ['responsible']];
    }
    ...
    {
        return "/api/v1/poultry-houses/{$house->id}/maintenances";
    }
